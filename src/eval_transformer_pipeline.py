...
def evaluate_transformer_rouge(val_csv_path, seq_len=20, num_samples=200,
    ...
    Args:
        ...
        num_samples:    количество примеров для оценки (0 = весь датасет)
    ...
    total = len(rows) if num_samples == 0 else num_samples
    for count, row in enumerate(tqdm(rows[:total], total=total, desc='Transformer eval')):
